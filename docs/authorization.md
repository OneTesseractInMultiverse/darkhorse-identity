...
```mermaid
flowchart LR
    Roles["Current assigned role capabilities"] --> Intersect["Set intersection"]
    Resource["Resource capabilities"] --> Intersect
    Ceiling["Immutable issuance ceiling"] --> Intersect
    Scopes["Current allowed scope capabilities"] -->|OAuth only| Intersect
    Intersect --> Effective["Effective capabilities"]
    Effective --> Check["Required capability present"]
```

Role and scope unions are computed within their own categories. The final
intersection cannot grant authority absent from any required bound.
...
It checks credential subject, target, revocation, principal epoch, activation/expiry, principal status, application/resource status, and role assignments. OAuth grants require a currently active client, an active owning application, and explicitly registered scopes for the requested resource. Client ownership and target application may differ when registration and principal assignments explicitly permit access.
...
Capabilities from multiple assigned roles are combined before applying the limits. Capabilities from multiple selected scopes are combined. For example, a writer role grants `read` and `update`, but a token with only a `read` scope can exercise only `read`. A scope cannot compensate for a missing role.
...
Time boundaries are inclusive for `valid_from` and exclusive for `expires_at`. An expiry at or before activation is invalid. OAuth access grants require expiry. Personal keys can have no expiry. The [key-issuance use case](personal-api-keys.md) enforces the organization's expiration policy.
...
Catalog validation happens when constructing a policy snapshot. Evaluation reuses immutable catalogs and indexed definitions. It visits supplied assignments, scopes, and capability sets without I/O. Planned cache adapters require fresh authoritative revision and status checks
before reusing versioned computations. They must bound input sizes and policy sizes, avoid rebuilding a whole organization catalog for every request, and measure the full request path before claiming throughput or latency targets.
...
[catalog validation](../crates/domain/src/authorization/catalog.rs),
[evaluation](../crates/domain/src/authorization/evaluate.rs),
[delegation](../crates/domain/src/authorization/delegation.rs).
