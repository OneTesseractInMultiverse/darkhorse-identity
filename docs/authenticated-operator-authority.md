...
Supported security writers acquire the exclusive security fence. Catalog and
directory readers hold its shared mode; single-account operations and catalog
...
| `account show`: record or not found                                                                                 | Exclusive; before target lock, after target read, final check                                                                        | [Account coordinator](../crates/adapters/src/postgres/operator_accounts.rs)                                                                   | [Account tests](../crates/adapters/tests/integration/operator_accounts.rs), [authority matrix](../crates/adapters/tests/integration/operator_authority.rs)                                                                                         |
...
row-lock holding interval through more primary reads. Account show still takes
the exclusive fence. Read-contention optimization is tracked separately in #32;
it must retain this contract. Use the comparable operator workload described in
