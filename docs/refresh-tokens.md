...
Confidential clients can explicitly activate opaque refresh tokens through the
...
session expires independently of continued application refresh requests.
The client must then start a new authorization flow. A still-valid SSO session
can avoid another password prompt. A full family requires new authorization.
...
during the family lifetime revokes that family, including every access token issued
...
denies all families bound to it. Other browser sessions remain independent.
...
Revoking one access token remains access-only. Authorization-code replay revokes the corresponding family. Wrong-client, wrong-issuer and wrong-resource
...
sequenceDiagram
    ...
    C->>D: Refresh token generation N
    D->>P: Lock root code and check live family
    P->>P: Consume N and commit N+1 with new access
    D-->>C: Replacement refresh and access tokens
    C->>D: Reuse consumed generation N
    D->>P: Commit family revocation and audit
    D-->>C: Reject replay
    Note over C,P: Generation N+1 and family access tokens now fail
```

Clients serialize refresh requests. A lost successful response requires a new
authorization flow. Replaying the old token can invalidate the winning response.
...
Initial code exchanges recheck client, session and code expiry after signing.
...
[refresh policy](../crates/domain/src/refresh.rs),
[family transactions](../crates/adapters/src/postgres/tokens/refresh/mod.rs),
[maintenance](../crates/adapters/src/postgres/tokens/refresh/maintenance.rs).
