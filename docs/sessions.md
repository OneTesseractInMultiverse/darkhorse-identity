...
reduces access: possession of a stolen live session can also end that user's other
...
shared policy and use cases while enforcing their own approved actor assurance.
...
- `signed_out`: the presented handle was ended. The actor reference identifies
  that handle's session, even if expired or otherwise unable to authenticate.
...
`session_audit`, usage of its identity sequence and lock-capable access to
`security_state`. Production role separation remains deployment work. Migration
ownership must stay separate from routine application access.
...
Invitation, verification, recovery, email changes, protected email delivery,
account-security notifications and their assurance decisions remain open. Complete
