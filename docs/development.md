...
Open **https://localhost:8443**. The portal provides password sign-in, profile editing, personal keys, and
...
[email configuration](email-verification.md). The [catalog console](catalog-administration.md) manages applications, clients,
resources, scopes, roles, capabilities, and explicit bindings.

For subsequent starts, run `make db-up redis-up`, inspect `make limiter-status`,
then run `make dev-login` with an active limiter. A changed Redis generation
...
stops its dependency containers and preserves their data.
...
To activate OAuth/OIDC and login, follow [provider setup](provider.md#configuration-and-development)
and use `make dev-provider` instead. Plain `make dev` provides the foundation
preview with authentication off by default.

```mermaid
flowchart LR
    Browser["Browser: localhost 8443"] --> Caddy["Local Caddy TLS proxy"]
    Caddy -->|API and protocol routes| Rust["Rust: 127.0.0.1 port 3001"]
    Caddy -->|Frontend and hot reload| Vite["Vite: 127.0.0.1 port 5173"]
    Rust --> DB[("Local Percona PostgreSQL")]
    Rust --> Redis[("Local Redis limiter")]
```

This is the host development topology. Packaged deployments serve static assets
from Rust and have no Vite process.
...
`make https-setup` validates `config/Caddyfile` and creates a project-local CA in `.local/pki`, with restrictive permissions and implicit trust installation disabled. `make dev` requires that setup. The CA survives normal restarts and `make clean`. Do not delete it before removing trust in its certificate.
...
The local origin addresses host processes. Inside a container, `localhost` names
that container. The [Compose](compose.md) and [Kubernetes](kubernetes.md) fixtures
provide canonical-host routing and public CA distribution for their clients.
An internal service URL must never become a second issuer.
...
Public origin must be HTTPS with no credentials, path, query, or fragment. The development topology uses fixed ports. Changing server settings alone does not reconfigure the proxy. The active provider binds this origin as its issuer. Discovery requires an active signing key. Configuration errors never print supplied values. The HTTP preview needs no credentials. Explicit database operator commands use separate settings documented in [persistence](persistence.md).
