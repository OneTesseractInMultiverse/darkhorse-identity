...
The optional nonsecret configuration field `defaultLocale` accepts `en` or `es`
and defaults to English. The builder supplies `DARKHORSE_DEFAULT_LOCALE` to runtime
Pods. Change the configuration and review/reapply the generated Deployment to roll
out a new fallback. Existing accounts and credentials are unaffected. See
[language preferences](localization.md) for precedence.
...
| Per-server connections                     | PostgreSQL 5, limiter 4 per pool (8 total), cache 2 |
| Per-operator connections                   | PostgreSQL 2, limiter 1, cache 2                    |
| Conservative namespace connection envelope | PostgreSQL 20, limiter 32, cache 8                  |
