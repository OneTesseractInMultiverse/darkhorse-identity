...
The application namespace starts with default-deny ingress and egress. Server Pods allow TCP 8443 only from the explicitly selected ingress namespace. Both server and operator Pods allow DNS TCP/UDP 53 to `kube-system` Pods labelled `k8s-app=kube-dns`, and database/cache/limiter traffic only to the selected backend namespace with both labels:
...
| `darkhorse-operator-secrets` | `owner-db`: schema-owner PostgreSQL URL with `sslmode=verify-full`; the same `login-key`, `wrap-key`, `cache-url`, `limiter-url`, `ca`; separate `limiter-admin-url` recovery credential                                                                    |
