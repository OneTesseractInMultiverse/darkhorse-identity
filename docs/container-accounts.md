...
The [access-catalog guide](operator-access-catalog.md) covers resources, scopes, roles
and capabilities through the same four targets.
The parallel [catalog targets](operator-catalog.md#make-launchers) list and inspect applications
and clients, and support [application creation and updates](operator-applications.md)
[client configuration updates](operator-clients.md), and [credential inventory/retirement](operator-client-secrets.md),
through the same launch and workload lifecycle. Both groups require a
...
Account and application commands take a JSON object containing `email`, `password`
and, for mutations, `reason`. Client updates take the nested authentication and
configuration document in the [client guide](operator-clients.md#invocation-and-complete-input). Deliver input
...
launcher passes stdin directly to the transport. Rust validates the 16 KiB limit (32 KiB for the complete client-update document)
...
The same runtime configuration supports native [application/client catalog reads](operator-catalog.md#process-and-container-operation)
[application writes](operator-applications.md), [client updates](operator-clients.md)
and [credential inventory/retirement](operator-client-secrets.md). Their protected-input and bounded
...
| Resource/scope/role/capability listing through all four catalog targets           | Runtime database/limiter credentials and fresh administrator proof; explicit application or all-definitions selection       | Bounded metadata and primary audit; demotion fails; one-shot paths work with HTTP stopped                                                             | Native `0`; Make wraps failure as `2`; Compose/Kubernetes fixtures          |
...
| Application create/update through all four catalog Make targets                   | Same authority, complete specification, reason and confirmation; update requires revision                                   | Application and both audits commit; stale revisions and demotion leave configuration unchanged; HTTP may be stopped                                   | JSON, `0`; conflicts/denials wrapped by Make as `2`. Both deployment suites |
| Client configuration update through all four catalog Make targets                 | Same authority, complete protected configuration, reason, confirmation and revision                                         | Exact callback/refresh replacement and both audits commit; stale revisions and demotion preserve configuration; one-shot paths work with HTTP stopped | JSON, `0`; conflicts/denials wrapped by Make as `2`. Both deployment suites |
