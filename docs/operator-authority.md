...
| `operator client secret list/retire`                                | Runtime database, active limiter, fresh administrator password; retirement also requires confirmation, reason and revision | Read audit before metadata release; retirement, revision and both audits commit together |
| `operator client update APPLICATION_ID CLIENT_ID REVISION`          | Runtime database, active limiter, fresh administrator password, confirmation/reason and revision                           | Shared registration configuration replacement and both audits commit together            |
...
and [credential inventory/retirement](operator-client-secrets.md). This grants no
application-resource authority. Deployments must restrict database credential access.
