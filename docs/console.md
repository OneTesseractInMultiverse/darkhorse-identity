...
- **Deactivate** and **Reactivate** require confirmation. Deactivation retains
  the profile and assignments but invalidates access. Reactivation preserves
...
The full profile page supports
[extended attributes and private picture uploads](profiles-and-media.md).
...
`change` object whose `kind` is `names`, `status`, or `role`. Role changes supply `application_id`, `role_id`, `assigned`, and `policy_revision`.
...
Writes require authentication within five minutes, exact same-origin
...
[directory policy](../crates/domain/src/admin_directory.rs),
[application port](../crates/application/src/admin_directory.rs).
