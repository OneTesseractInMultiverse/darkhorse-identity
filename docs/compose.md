...
| `postgres` | TLS only over private database network; named persistent volume          | Separate bootstrap superuser, schema owner, and runtime login       |
...
| `operator` | Explicit one-shot container; no HTTP listener or published port          | Schema-owner login and limiter recovery credential                  |
