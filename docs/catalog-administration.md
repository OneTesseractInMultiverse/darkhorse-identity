...
```mermaid
flowchart LR
    Definition["Shared role or capability definition"] --> Binding["Explicit application binding"]
    Binding --> Assignment["Principal role assignment within application"]
    Resource["Resource capability exposure"] --> Scope["Scope capability limit"]
    Assignment --> Evaluate["Current authorization intersection"]
    Scope --> Evaluate
```

Shared definitions require explicit bindings. A scope limits delegation and
never creates a role assignment.
...
`application_id`. Scope details require `resource_id`.
...
rechecks authority after reference reads and locks. Registration rechecks
...
[catalog policy](../crates/domain/src/admin_catalog.rs),
[application port](../crates/application/src/admin_catalog.rs).
