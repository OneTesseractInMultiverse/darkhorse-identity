-- Directory reads release results only after current authority and this audit commit.
-- Search text and returned names/emails are intentionally not retained here.
CREATE TABLE operator_directory_audit (
 id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 operation_id uuid NOT NULL UNIQUE CHECK(operation_id<>'00000000-0000-0000-0000-000000000000'),
 command text NOT NULL DEFAULT 'account.list' CHECK(command='account.list'),
 ...
 result text NOT NULL CHECK(result IN ('read','denied')),
...
CREATE INDEX operator_directory_audit_actor ON operator_directory_audit(actor_id,id);
CREATE TRIGGER operator_directory_audit_immutable BEFORE UPDATE OR DELETE ON operator_directory_audit
