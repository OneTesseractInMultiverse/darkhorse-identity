...
fn location(target: ReturnTo, error: Error) -> Result<String, Error> {
...
pub(super) fn redirect(target: ReturnTo, error_code: Error) -> Response {
    match location(target, error_code) {
        Ok(uri) => Redirect::to(&uri).into_response(),
        Err(e) => error(e),
    }
}
pub(super) fn return_json(target: ReturnTo, error_code: Error) -> Response {
    match location(target, error_code) {
