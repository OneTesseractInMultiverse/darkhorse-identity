...
pub fn parse_catalog(raw: &str) -> Result<darkhorse_domain::admin_catalog::Query, InvalidQuery> {
    if raw.len() > 2048 {
        return Err(InvalidQuery);
    }
    let mut seen = BTreeSet::new();
    ...
    for (key, value) in url::form_urlencoded::parse(raw.as_bytes()) {
        ...
        match key.as_ref() {
            ...
            "search" => search = value.trim().into(),
            "after" => {
                let id = uuid::Uuid::parse_str(&value).map_err(|_| InvalidQuery)?;
                if id.to_string() != value {
                    return Err(InvalidQuery);
                }
                after = Some(std::num::NonZeroU128::new(id.as_u128()).ok_or(InvalidQuery)?);
            }
            _ => return Err(InvalidQuery),
        }
    }
    let criteria = parse(&filters.finish())?;
    let q = darkhorse_domain::admin_catalog::Query {
        search,
        active: criteria.status.map(|s| s == AccountStatus::Active),
        after,
        limit: criteria.limit,
    };
    q.validate().map_err(|_| InvalidQuery)?;
    Ok(q)
}
