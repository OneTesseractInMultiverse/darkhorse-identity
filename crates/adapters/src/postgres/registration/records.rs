...
pub(super) async fn application(
    ...
    let row = sqlx::query(
        "SELECT a.*,p.email FROM applications a JOIN principals p ON p.id=a.owner_id WHERE a.id=$1",
    )
...
pub(super) async fn client(
    ...
    let row = sqlx::query("SELECT * FROM oauth_clients WHERE id=$1 AND application_id=$2")
        .bind(uuid(id.as_u128()))
    ...
    let redirects: Vec<String> =
        sqlx::query_scalar("SELECT uri FROM client_redirects WHERE client_id=$1 ORDER BY uri")
            .bind(uuid(id.as_u128()))
            .fetch_all(&mut **tx)
            .await
            .map_err(storage)?;
    let resources: Vec<Uuid> = sqlx::query_scalar(
        "SELECT resource_id FROM client_resources WHERE client_id=$1 ORDER BY resource_id",
    )
    .bind(uuid(id.as_u128()))
    .fetch_all(&mut **tx)
    .await
    ...
    let scopes: Vec<Uuid> = sqlx::query_scalar(
        "SELECT scope_id FROM client_scopes WHERE client_id=$1 ORDER BY scope_id",
    )
    .bind(uuid(id.as_u128()))
    .fetch_all(&mut **tx)
    .await
    ...
    assemble_client(&row, application, id, redirects, resources, scopes, secrets)
}
fn assemble_client(
    row: &PgRow,
    application: ApplicationId,
    ...
    Ok(ClientRecord {
        ...
        revision: number(row, "revision")?,
        spec,
        secrets: secrets.iter().map(secret).collect::<Result<_, _>>()?,
    })
}
