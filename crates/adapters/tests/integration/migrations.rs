...
async fn lost_intent_step_and_final_commit_replies_are_inspectable() {
    for commit in [1, 2, 3, 4, 5] {
        ...
        assert_eq!(record.completed_ms.is_some(), commit == 5);
        assert_eq!(history(&db).await, 23 + (commit - 1).min(3) as i64);
        store.close().await;
        db.pool.close().await;
    }
}
