...
async fn fresh_upgrade_and_noop_preserve_baseline_and_step_receipts() {
    for version in [0, 22, 24, 25, 26, 27, 28, 29, 30, 31, 32] {
...
async fn lost_intent_step_and_final_commit_replies_are_inspectable() {
    for commit in 1..=11 {
        ...
        assert_eq!(record.completed_ms.is_some(), commit == 11);
        assert_eq!(history(&db).await, 23 + (commit - 1).min(9) as i64);
        store.close().await;
        db.pool.close().await;
    }
}
